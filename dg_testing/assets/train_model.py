...
@multi_asset(
    ...
    ins={
        "preprocessed_data": AssetIn(key=AssetKey("movies_users"), input_manager_key="postgres_io_manager"),
        "review_stats": AssetIn(key=AssetKey("review_stats"), input_manager_key="postgres_io_manager"),
        "user_critic_reviews": AssetIn(key=AssetKey("user_critic_reviews"), input_manager_key="postgres_io_manager"),
    },
    outs={
...
def preprocessed_data(
    ...
    user_critic_reviews: pd.DataFrame
):
    """
    Preprocesses the data for training the model.
    Enriquecido con review_stats y user_critic_reviews.
    """
    df = preprocessed_data.copy()

    # Merge con review_stats por movie_title o movie_id (dependiendo de tus columnas)
    df = df.merge(review_stats, on='movie_title', how='left')

    # Merge con user_critic_reviews para traer user_score y critic_score
    df = df.merge(user_critic_reviews[['user_id', 'movie_id', 'user_score', 'critic_score']], 
                  on=['user_id', 'movie_id'], how='left')

    # Llenamos NaNs con 0 o alguna estrategia más avanzada
    df = df.fillna(0)

    # Codificamos user_id y movie_id
    ...
    df["encoded_user_id"] = df["user_id"].map(user2Idx)
    df["encoded_movie_id"] = df["movie_id"].map(movie2Idx)

    # Suponemos que el target sigue siendo "rating"
    df["rating"] = df["user_score"]
...
def model_trained(context, config:MyModelConfig, X_train, y_train, user2Idx, movie2Idx):
    ...
    mlflow.set_experiment("RecomendadorMovies")
